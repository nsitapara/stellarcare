...
# Check if Docker Compose is installed (it's included in docker-compose-plugin)
...
# Configure Nginx to run as root user (for accessing files)
...
server {
    ...
    # Django static files - serve directly
    location /static/ {
    ...
    # Django media files - serve directly
    location /media/ {
...
# Enable the site
ln -sf /etc/nginx/sites-available/$DOMAIN /etc/nginx/sites-enabled/ || error_exit "Failed to enable Nginx site."
...
# At this point, certbot has automatically modified the Nginx configuration to handle SSL
...
# Check Django logs
echo "[INFO] Checking Django logs..."
